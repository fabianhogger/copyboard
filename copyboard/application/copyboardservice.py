...
class CopyboardService:
    ...
    def __init__(
        ...
        self._prepared_stack_clipping_id: str | None = None
    ...
    def remove_expired_clippings(self) -> None:
        ...
        if self._prepared_stack_clipping_id is not None and any(
            clipping.id == self._prepared_stack_clipping_id for clipping in removed
        ):
            self._prepared_stack_clipping_id = None
    ...
    def delete_clipping_by_id(self, clipping_id: str) -> None:
        ...
        if clipping is not None and self._history.remove_clipping_by_id(clipping_id):
            ...
            if self._prepared_stack_clipping_id == clipping.id:
                self._prepared_stack_clipping_id = None
                self._prepare_newest_clipping_for_stack_paste(clear_clipboard_if_empty=True)
    ...
    def consume_prepared_clipping_after_paste(self) -> None:
        ...
        if not self._stack_paste_mode_enabled or self._prepared_stack_clipping_id is None:
            return

        clipping_id = self._prepared_stack_clipping_id
        self._prepared_stack_clipping_id = None
        clipping = self._history.find_clipping_by_id(clipping_id)
        if clipping is not None and self._history.remove_clipping_by_id(clipping_id):
            self._observers.notify_all(ClippingRemoved(clipping))
    ...
    def _prepare_newest_clipping_for_stack_paste(
        ...
        self._prepared_stack_clipping_id = newest_clipping.id
